...
interface ChatInputTextAreaProps extends React.ComponentProps<"textarea"> {
    value?: string;
    onChange?: React.ChangeEventHandler<HTMLTextAreaElement>;
    onSubmit?: () => void;
    variant?: "default" | "unstyled";
...
const ChatInputTextArea = forwardRef<HTMLTextAreaElement, ChatInputTextAreaProps>(({
    ...
    return (
        <textarea
            ...
            className={cn(
                "flex w-full rounded-lg bg-transparent px-3 py-2 text-sm text-foreground placeholder:text-muted-foreground focus-visible:outline-none disabled:cursor-not-allowed disabled:opacity-50 max-h-[400px] min-h-0 resize-none overflow-x-hidden",
                variant === "unstyled" &&
                "border-none focus-visible:ring-0 focus-visible:ring-offset-0 shadow-none",
                variant === "default" &&
                "border border-input shadow-sm focus-visible:border-ring focus-visible:ring-[3px] focus-visible:ring-ring/20",
                className,
            )}
            rows={rows}
        />
